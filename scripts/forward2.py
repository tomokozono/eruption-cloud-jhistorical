#!/usr/bin/env python3
"""
Forward simulation (v2): fixed r0 = 50, 100, 150 m in rows 1–3.

Differences from forward.py:
  - r0 is fixed to [50, 100, 150] m regardless of catalog settings.
  - Observed plume height (z_target_m) is shown as a horizontal dashed line
    in the velocity and distance panels.
  - Wind profile remains in the top-right panel.

Usage:
    uv run python scripts/forward2.py --eruption Tokachi1962
    uv run python scripts/forward2.py --eruption all
"""
import argparse
import sys
...
def run_forward2(cfg: dict, eruption_key: str, output_dir: Path):
    ...
    for i, r0 in enumerate(R0_LIST):
        ...
        for col in (0, 1):
            axs[i, col].axhline(z_target, color="black", linewidth=1.2,
                                linestyle="--", label=f"observed {int(z_target/1000)} km")
    ...
    for i in range(n_rows):
        for j in range(2):
            axs[i, j].set_ylim(0, 25_000)

    plt.tight_layout()

    out_png = output_dir / f"plume_forward2_{eruption_key}.png"
...
def main():
    ...
    for key in eruptions:
        ...
        out_dir = ROOT / args.output_dir / key / "forward2"
        out_dir.mkdir(parents=True, exist_ok=True)
        run_forward2(catalog[key], key, out_dir)
